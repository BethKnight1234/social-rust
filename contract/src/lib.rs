...
pub struct SocialNetworking {
    ...
    likes_by_user_id: UnorderedMapu128<AccountId, Vec<Post>>,
    posts_by_tag: UnorderedMap<String, Vec<Posts>>
}

impl Default for SocialNetworking {
    fn default() -> Self {
        Self {
            posts: UnorderedMapu128::new(b'm'),
            number_of_posts: 0,
            likes_by_user_id: UnorderedMapu128::new(b'n'),
            posts_by_tag: UnorderedMapu128::new(b'o')
        }
    }
}
...
imp SocialNetworking{
    ...
    pub fn add_post_to_my_liked(&mut self, sender_id:AccountId, post: Post) {
        if let None = self.likes_by_user_id.get(&sender_id) {
            ...
            likes = self.likes_by_user_id.get(&sender_id).unrwap();
        }
        likes.push(post);
    ...
    pub fn add_posts_by_tag(&mut self, post: Post, tags: Vec<Strings>) {
        for tags in tags {
